...
[![Version: 1.0.1](https://img.shields.io/badge/Version-1.0.1-informational?style=flat-square)](https://github.com/HenriqZimer/shadps4-helm-chart)
...
early PlayStation 4 emulator, served as a full desktop over the browser via KasmVNC.
...
```bash
git clone https://github.com/HenriqZimer/shadps4-helm-chart.git
cd shadps4-helm-chart

helm package chart/
helm install shadps4 ./shadps4-1.0.1.tgz
```
...
### shadPS4 crashes when launching a game (SIGBUS)

If the dashboard/UI works but a game crashes on boot (check the container logs for
`Unhandled SIGBUS`), shadPS4's JIT recompiler may need more `/dev/shm` than the small
...
starts when `/dev/shm` is too small); not independently confirmed on shadPS4, but worth trying
...
> [!NOTE]
> Unlike the other sibling charts, the upstream [linuxserver.io shadPS4 docs](https://docs.linuxserver.io/images/docker-shadps4/)
> state this image **only runs in Wayland mode and should be run with a GPU** — treat `gpu.enabled: true`
> as required, not optional, for this chart.
...
[linuxserver.io GPU Configuration docs](https://docs.linuxserver.io/images/docker-shadps4/#gpu-configuration)):
...
If shadPS4 itself crashes with `Unhandled SIGBUS` in the container logs on boot (i.e. the dashboard works
but launching a game doesn't), see `shmSize` below and `seccompUnconfined` above — try those first, it's
usually not a GPU problem.

### RomM emulator streaming — not available for shadPS4
...
third-party Docker Mod that adds an HTTP broker RomM talks to. **No such mod exists for shadPS4**:
[github.com/LoneAngelFayt/shadps4-romm-integration](https://github.com/LoneAngelFayt/shadps4-romm-integration)
...
today — leave it `false` and use shadPS4 directly through its own desktop (`service.httpsPort`),
...
| `image.repository` | `lscr.io/linuxserver/shadps4` | Container image. |
